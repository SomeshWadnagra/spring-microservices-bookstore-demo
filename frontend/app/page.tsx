...
// We no longer use ADD_BOOK via GraphQL, so this can be removed if you want.
// const ADD_BOOK = gql`
...
const Home = () => {
  ...
  const [newBook, setNewBook] = useState<Book>({
    ...
    description: '',
    price: 0,
    imageUrl: '' // New for S3 Image URL
  });

  // NEW: state for file to upload
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Ensure price stays a number
    if (name === 'price') {
      setNewBook({ ...newBook, price: Number(value) });
    } else {
  ...
  const handleAddBook = async () => {
    ...
    try {
      // Payload WITHOUT imageUrl – backend will set it after S3 upload
      const bookPayload = {
        name,
        description,
        price
      ...
      if (!response.ok) {
        ...
        setValidationError('Failed to create book. Please check logs.');
        return;
      }

      await response.json(); // we don't use the response object directly here
      await refetchBooks();
      ...
      setValidationError('Unexpected error while adding book.');
    }
  };
    ...
    <div className="container mx-auto p-4 bg-gray-100 min-h-screen relative">
      ...
      <section className="mb-12 p-6 bg-white shadow-lg rounded-md">
        ...
        <p className="mb-4 text-gray-600">
          Please enter the name, description, and price of the book you would like to add, then
          choose an image and press the Add Book button.
        </p>
      ...
      <section className="mb-12 p-6 bg-white shadow-lg rounded-md">
        ...
        <p className="mb-4 text-gray-600">
          Enter the details for the new author and click &apos;Add Author&apos; to save.
        </p>
        <div className="flex flex-col space-y-4">
          <input
            className="border border-gray-300 p-2 rounded-md"
      ...
      <section ref={orderSectionRef} className="mb-12 p-6 bg-white shadow-lg rounded-md">
        ...
        <p className="mb-4 text-gray-600">
          Note that Design Patterns is in stock so the order will be placed successfully; however
          Mythical Man Month is not in stock.
        </p>
            ...
            {availableBooks.map((book) => (
              <button
                ...
                className={`border p-2 rounded-md ${
                  selectedBook.skuCode === book.skuCode
        ...
        <ul className="list-none space-y-4">
          <li>
            ...
            <p className="text-gray-600 mb-2">
              Eureka is a service registry for service discovery in cloud environments.
            </p>
            <a
          ...
          <li>
            ...
            <p className="text-gray-600 mb-2">
              Zipkin is a distributed tracing system for monitoring and troubleshooting microservices.
            </p>
            <a
              className="text-blue-500 hover:underline"
              href="http://localhost:9411/zipkin/"
              target="_blank"
              rel="noopener noreferrer"
            >
          ...
          <li>
            ...
            <p className="text-gray-600 mb-2">
              Grafana is a visualization tool for monitoring and analyzing metrics.
            </p>
            <a
              className="text-blue-500 hover:underline"
